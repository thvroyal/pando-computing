  ...
  function handlePandoMessages(channel) {
    function parse(data) {
      ...
      if (message.type === "DATA-CHANNEL-SIGNAL") {
        log(
          "channel(" + idSummary(channel.id) + ") received DATA-CHANNEL-SIGNAL"
        );
  ...
  function sendSummary() {
    var summary = {
      ...
      performance: {
        ...
        cpuUsageStats: performanceStatus.cpuUsageStats || {
          minimum: 0,
          average: 0,
          maximum: 0,
          "standard-deviation": 0,
    ...
    for (var s in latestStatus) {
      ...
      if (child.performance && child.performance.throughputStats) {
        ...
        stats["standard-deviation"] += Number(
          child.performance.throughputStats["standard-deviation"]
        );
      ...
      if (child.performance && child.performance.cpuUsageStats) {
        ...
        stats["standard-deviation"] += Number(
          child.performance.cpuUsageStats["standard-deviation"]
        );
      ...
      if (child.performance && child.performance.dataTransferStats) {
        ...
        stats["standard-deviation"] += Number(
          child.performance.dataTransferStats["standard-deviation"]
        );
  ...
  node.on("child-connect", function (child) {
    ...
    child.on("data-channel-signal", function (signal) {
      if (dataChannel) {
        ...
        log(
          "WARNING: Missed data-channel-signal from child(" +
            idSummary(child.id) +
            ")"
        );
      }
    ...
    child.on("status", function (status) {
      if (limitedChannel) {
        ...
        status.limit = limit;
        log(
          "updating child(" + idSummary(child.id) + ") limit to " + status.limit
        );
    ...
    child.on("error", function (err) {
      log(
        "child(" +
          idSummary(child.id) +
          ") control channel failed with error: " +
          err
      );
      if (limitedChannel) limitedChannel.source(true, function () {});
      removeChild(child);
    ...
    dataChannel
      ...
      .on("error", function (err) {
        log(
          "child(" +
            idSummary(child.id) +
            ") data channel failed with error: " +
            err
        );
    ...
    pull(
      ...
      pull.map(function (x) {
        try {
          ...
          console.error("Error occurred during gunzip");
          return "Error value";
        }
      }),
      pull.through(function () {
